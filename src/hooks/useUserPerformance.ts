...
export function useUserPerformance(): PerformanceData {
  ...
  useEffect(() => {
    ...
    const loadPerformance = async () => {
      ...
      // Score: média ponderada de ações nos últimos 7 dias vs meta (10 ações/dia)
      ...
      // Streak: dias consecutivos retroativos a partir de HOJE
      // O banco só tem linhas para os dias com ações (pois a VIEW agrupa). 
      // Precisamos checar se as datas reais batem, preenchendo os defaults
      let streak = 0
      ...
      while (streak < 90) { // arbitrary limit
        ...
        if (actions > 0) {
          ...
          // If we are checking today and today is 0, we shouldn't break the streak immediately if yesterday was active.
          // Slack and Duolingo style: streak is active if you did it yesterday.
          if (checkKey === todayStr) {
             cursorDate = subDays(cursorDate, 1)
             // don't increment streak for today since 0, but check yesterday
