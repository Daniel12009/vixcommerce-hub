...
async function invokeSheets(spreadsheetId: string, range: string, values: any[][], action: 'append' | 'write' | 'dedup_write' = 'append', dateColumn?: number, contaColumn?: number) {
  ...
  if (rawTab) {
    ...
    } catch { /* tab may already exist ÔÇö OK */ }
  }
...
// UTILIT├üRIOS ÔÇö equivalentes exatos ├ás fun├º├Áes Python
// ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ

const MAPA_ESTADOS: Record<string, string> = {
  'AC':'Acre','AL':'Alagoas','AP':'Amap├í','AM':'Amazonas','BA':'Bahia',
  'CE':'Cear├í','DF':'Distrito Federal','ES':'Esp├¡rito Santo','GO':'Goi├ís',
  'MA':'Maranh├úo','MT':'Mato Grosso','MS':'Mato Grosso do Sul','MG':'Minas Gerais',
  'PA':'Par├í','PB':'Para├¡ba','PR':'Paran├í','PE':'Pernambuco','PI':'Piau├¡',
  'RJ':'Rio de Janeiro','RN':'Rio Grande do Norte','RS':'Rio Grande do Sul',
  'RO':'Rond├┤nia','RR':'Roraima','SC':'Santa Catarina','SP':'S├úo Paulo',
  'SE':'Sergipe','TO':'Tocantins',
};

function traduzirEstado(entrada: string): string {
  if (!entrada) return 'N├úo Identificado';
...
async function consultarFrete(
  ...
  try {
    ...
    // C├üLCULO REAL: Custo total da etiqueta - Valor pago pelo comprador
    ...
    if (ltype === 'fulfillment') {
      ...
      tipo_log = 'Mercado Envios Ag├¬ncia';
    }
...
async function processarVendaMLSingle(
  ...
  try {
    ...
    for (const item of orderItems) {
      ...
      if (!estado) estado = 'N├úo Identificado';

      const fee = parseFloat(String(item.sale_fee ?? 0)) || 0;
      const fee_total_neg = -1 * (fee * qtd);

      // Bug fix: prefixar datas com ap├│strofo para for├ºar texto no Sheets
      const data_criacao = `'${formatarDataBR(venda.date_created || '')}`;
      const data_fechamento = `'${formatarDataBR(venda.date_closed || '')}`;
      const id_venda_planilha = `'${id_referencia_pedido}`;  // ap├│strofo ÔåÆ for├ºa texto no Sheets
      const id_venda_banco    = id_referencia_pedido;          // n├║mero limpo para o banco
      ...
      linhasSheets.push([
        ...
        'Mercado Livre',
        ml_id,
        String(listing_type_id).toLowerCase().includes('gold_special') ? 'Cl├íssico' : 'Premium',
        '',
        tipo_log,
        preco.toFixed(2),
        String(qtd),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'get_today_orders') {
      ...
      // Get today's date range in S├úo Paulo time (UTC-3)
      const now = new Date();
      const spOffset = -3 * 60; // S├úo Paulo UTC-3
    ...
    // ÔöüÔöüÔöü FICHA T├ëCNICA: Listing management actions ÔöüÔöüÔöü
    ...
    if (action === 'list_seller_items') {
      ...
      // Post-filter: ML search index can be stale ÔÇö remove items that don't match requested status
    ...
    if (action === 'create_item') {
      ...
      // Convert shipping dimensions object ÔåÆ ML string format "HxWxL,Weight"
      ...
      if (new_item.shipping?.dimensions && typeof new_item.shipping.dimensions === 'object') {
        ...
        if (d.height && d.width && d.length && d.weight) {
          ...
          // Incomplete dimensions ÔÇö remove to avoid errors
      ...
      // Smart retry loop ÔÇö handles ML validation errors automatically
      let result: any;
      let attempts = 0;
      const maxAttempts = 4;
      ...
      while (attempts < maxAttempts) {
        ...
        try {
          ...
          // Handle body.invalid_fields ÔÇö User Products categories don't accept title/condition
          ...
          // Handle missing_conditional_required ÔÇö required attributes like GTIN
            ...
            if (attrMatch) {
              ...
              for (const attrId of missingAttrs) {
                ...
                if (!new_item.attributes.find((a: any) => a.id === attrId)) {
                  console.log(`[CREATE] Auto-adding required attribute: ${attrId} = "N├úo se aplica"`);
                  new_item.attributes.push({ id: attrId, value_name: 'N├úo se aplica' });
                }
              }
              continue; // retry
            }
          ...
          // Handle shipping dimension errors ÔÇö strip dimensions and retry
    ...
    if (action === 'get_ads_data') {
      // ML Product Ads API ÔÇö Official 2026 docs
      ...
      for (const account of accounts) {
        try {
          ...
          // ÔöüÔöüÔöü Step 1: Discover advertiser_id for ALL product types ÔöüÔöüÔöü
          ...
          for (const adv of allAdvertisers) {
            ...
            // ÔöüÔöüÔöü Step 2: Fetch campaigns with metrics ÔöüÔöüÔöü
            ...
            // ÔöüÔöüÔöü Step 3: Fetch ad items with metrics ÔöüÔöüÔöü
    ...
    if (action === 'get_catalog_winner') {
      ...
      if (!catalogProductId) {
        return new Response(JSON.stringify({ catalog: false, message: 'Item n├úo ├® de cat├ílogo' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
    ...
    // ÔòÉÔòÉÔòÉ QUESTIONS (Atendimento) ÔòÉÔòÉÔòÉ
    ...
    if (action === 'answer_question') {
      ...
      const accounts = await accountsRes.json();
      if (!accounts?.length) throw new Error('Conta ML n├úo encontrada');
      const account = accounts[0];
    ...
    if (action === 'get_category_attributes') {
      ...
      // Busca autenticada ÔÇö retorna marcas registradas na conta do seller
      const accountsRes = account_id
        ? await supabaseFetch(`/ml_accounts?id=eq.${account_id}&ativo=eq.true`)
        : await supabaseFetch('/ml_accounts?ativo=eq.true');
      const accounts = await accountsRes.json();
    ...
    // ÔòÉÔòÉÔòÉ SYNC VENDAS ML ÔåÆ GOOGLE SHEETS ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
    if (action === 'sync_vendas') {
      ...
      if (todasVendas.length === 0) {
        return new Response(JSON.stringify({ mensagem: 'Nenhuma venda encontrada no per├¡odo.', linhas_escritas: 0 }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      if (loteLinhas.length > 0) {
        // Dedup: VendasML has date created at column 2 (index 2: "Data de Cria├º├úo")
        // But date could also be used at column 11 if there is a 'Data Ref' instead?
        // Let's use date created (index 2). Wait, earlier we saw VendasML has Data Ref at col 11 (Actually col 2 is Data Cria├º├úo, VendasML is what?).
        // For vendas, the data processed is 'dateFrom', which might be passed as a ref.
        // Actually earlier it was appending. Let's use column 2 (Data de Cria├º├úo) for VendasML which has `'DD/MM/YYYY`.
    ...
    // ÔòÉÔòÉÔòÉ PERFORMANCE CAT├üLOGO ML ÔåÆ GOOGLE SHEETS ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
    if (action === 'get_performance_catalog') {
      ...
      // PASSO 1: buscar TODOS os itens ativos da conta (sem restringir a full ou cat├ílogo)
      ...
      console.log(`[PERF] ${account.nome}: ${itemIds.length} itens Full cat├ílogo`);
      if (itemIds.length === 0) {
      ...
      if (healthInserts.length > 0) {
        ...
        for (let i = 0; i < healthInserts.length; i += 50) {
           ...
           if (!insertRes.ok) {
             ...
             console.log(`[HEALTH_INSERT] OK chunk ${i}ÔÇô${i + chunk.length} for ${account.nome}`);
           }
        }
      }
      ...
      // PASSO 6: escrever
      if (rows.length > 0) {
        const header = ['Plataforma', 'ID An├║ncio', 'SKU', 'T├¡tulo', 'Pre├ºo', 'Visitas', 'Vendas', 'Canceladas', 'Convers├úo %', 'Link', 'Conta', 'Data Ref'];
    ...
    // ÔòÉÔòÉÔòÉ VENDAS FULL 7D ÔåÆ GOOGLE SHEETS ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
    if (action === 'get_vendas_full_7d') {
      ...
      if (rows.length > 0) {
        ...
        } catch { /* ignora se aba n├úo existir */ }
    ...
    // ÔòÉÔòÉÔòÉ ADS FULL REPORT ÔåÆ GOOGLE SHEETS ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
    if (action === 'get_ads_full_report') {
      ...
      if (linhas_resumo.length > 0) {
        // For ADS resumo: [data, conta, valor] ÔÇö dedup by date (col 0) + conta (col 1)
    ...
    if (action === 'get_item_status') {
      ...
      try {
        ...
        if (res.status === 401) {
          ...
          // Item migrated to User Products ÔÇö follow redirect
    ...
    // ÔöÇÔöÇÔöÇ get_listing_types: batch-fetch catalog_listing for performance items ÔöÇÔöÇÔöÇ
              ...
              for (const item of (Array.isArray(items) ? items : [])) {
                ...
                if (b?.id) {
                  cachedTypes[b.id] = {
                    ...
                    listingType: b.catalog_listing === true ? 'Cat├ílogo' : 'Tradicional',
                  };
                }
              }
            } else {
